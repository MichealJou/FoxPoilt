...
- macOS
  - 推荐先用 `npm` 全局安装
  - `Homebrew` 安装入口预留为：`brew install michealjou/tap/foxpilot`
- Linux
  - 推荐先用 `npm` 全局安装
  - `GitHub Release` 安装脚本入口预留为：`curl -fsSL https://raw.githubusercontent.com/MichealJou/FoxPoilt/main/scripts/install.sh | sh`
- Windows
  - 推荐先用 `npm` 全局安装
  - `GitHub Release` 安装脚本入口预留为：`irm https://raw.githubusercontent.com/MichealJou/FoxPoilt/main/scripts/install.ps1 | iex`

说明：

- `npm install -g` 是系统级全局安装，不是项目内局部依赖安装
- 上面的 `Homebrew` 和 `GitHub Release` 已完成脚本与分发设计，但正式公网发布还在后续步骤里
