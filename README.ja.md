...
このリポジトリは CLI MVP 実装段階に入っています。コアとなる初期化、手動タスク管理、Beads ローカルスナップショットの取り込み・差分予覧・ローカル同期・ローカル環境診断・書き出し・単一タスクと一括書き戻し、次タスク選択、タスクメタデータ編集、走査提案タスク、実行者切替、優先度更新、タスク実行履歴、最小限の状態遷移制約の流れは利用可能で、次の反復では協調オーケストレーションを拡張していきます。
