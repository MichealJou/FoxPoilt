...
このリポジトリは CLI MVP 実装段階に入っています。コアとなる初期化、手動タスク管理、Beads ローカルスナップショット取り込み、次タスク選択、タスクメタデータ編集、走査提案タスク、実行者切替、優先度更新、タスク実行履歴、最小限の状態遷移制約の流れは利用可能で、次の反復では協調オーケストレーションを拡張していきます。
