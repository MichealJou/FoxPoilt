...
落地第一版 `foxpilot task push-beads` / `fp task push-beads`，让 FoxPilot 已导入的 `Beads` 任务可以把本地编辑结果反向写回仓库里的 `bd` 数据，并支持仓库级批量回写。
...
- 批量回写时必须显式限制在仓库级或全仓库级，不能退化成“默认全推”
