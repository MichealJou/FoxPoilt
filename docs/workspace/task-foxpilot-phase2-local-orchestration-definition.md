...
- 第二阶段 Workspace 页面选择模型
- 第二阶段事件时间线分组模型
- 第二阶段 Control Plane 状态汇总模型
- 第二阶段对象详情钻取模型
- 第二阶段桌面路由状态策略
- 第二阶段 Dashboard 卡片与异常入口策略
- 第二阶段 Tasks 列表视图策略
- 第二阶段 Runs 页面协同模型
...
- Workspace 不能同时像两个页面又像一个混乱清单
- Events 不能退化成扁平日志墙
- Dashboard 与 Control Plane 首页不能各自定义状态口径
- 摘要、右侧面板、详情页不能各自发明一套对象进入规则
- 页面状态恢复不能靠前端自行猜测
- Dashboard 不能堆成监控告警墙
- Tasks 不能退化成无限加列的大表
...
- Workspace 必须稳定处理项目 / 仓库选择与跳转关系
- Events 必须先按链路分组，再展示单条事件
- Control Plane 状态必须先标准化，再统一汇总
- 对象详情必须统一按 摘要 -> 右侧面板 -> 完整详情页 三层钻取
- 路由必须稳定承载可恢复状态，右侧面板尽量由路由对象恢复
- Dashboard 卡片、异常入口、Focus Queue 必须分工明确
...
- [x] 已补 Workspace 页面选择模型
- [x] 已补事件时间线分组模型
- [x] 已补 Control Plane 状态汇总模型
- [x] 已补对象详情钻取模型
- [x] 已补桌面路由状态策略
- [x] 已补 Dashboard 卡片与异常入口策略
- [x] 已补 Tasks 列表视图策略
- [x] 已补 Runs 页面协同模型
