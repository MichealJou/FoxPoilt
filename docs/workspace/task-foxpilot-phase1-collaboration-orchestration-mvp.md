...
- [x] 已补齐 `task diff-beads` 只读差异预览能力（文件 / 单仓库 / 全仓库）
- [x] 已补齐 `task sync-beads` 本地 bd CLI 同步能力（单仓库 / 全仓库）
- [x] 已补齐 `task doctor-beads` 本地环境诊断能力（单仓库 / 全仓库）
- [x] 已补齐 `task init-beads` 本地环境初始化能力（单仓库 / 全仓库 / dry-run）
- [x] 已补齐 `task push-beads` 本地回写能力（单任务 / 单仓库 / 全仓库 dry-run）
- [x] 已补齐 `task export-beads` 本地快照导出能力
- [x] 已形成“导入 / 同步 / 预演 / 收口 / 摘要 / 导出”的本地闭环
- [x] 已形成“导入 / 同步 / 预演 / 回写 / 收口 / 摘要 / 导出”的本地闭环
- [x] 已基于官方文档确认：当前 shell 场景采用 CLI-first 集成，不再把真实 `Beads` API 网络同步作为第一阶段完成门槛
