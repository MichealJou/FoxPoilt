...
describe('task suggest-scan CLI', () => {
  it('creates one scan suggestion task per registered repository', async () => {
    ...
    const result = await runCli(['task', 'suggest-scan'], { cwd: projectRoot, homeDir })
  ...
  it('skips repositories that already have unfinished scan suggestion tasks', async () => {
    ...
    const firstRun = await runCli(['task', 'suggest-scan'], { cwd: projectRoot, homeDir })
    expect(firstRun.exitCode).toBe(0)

    const secondRun = await runCli(['task', 'suggest-scan'], { cwd: projectRoot, homeDir })
  ...
  it('prints help and accepts fp alias', async () => {
    const result = await runCli(['task', 'suggest-scan', '--help'], { binName: 'fp' })
  ...
  it('returns exit code 4 when sqlite bootstrap fails', async () => {
    ...
    const result = await runCli(['task', 'suggest-scan'], {
      cwd: projectRoot,
      homeDir,
      failBootstrap: true,
    })
  ...
  it('returns structured json scan suggestion output', async () => {
    ...
    const result = await runCli(['task', 'suggest-scan', '--json'], { cwd: projectRoot, homeDir })
