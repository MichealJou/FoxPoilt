...
async function createManagedProjectWithTasks(): Promise<{
  ...
  const firstTask = await runCli(['task', 'create', '--title', '先做 task list'], {
    cwd: projectRoot,
    homeDir,
  })
  expect(firstTask.exitCode).toBe(0)

  const secondTask = await runCli(['task', 'create', '--title', '整理 task 输出'], {
    cwd: projectRoot,
    homeDir,
  })
...
describe('task list CLI', () => {
  ...
  it('supports filtering by status', async () => {
    ...
    const result = await runCli(['task', 'list', '--status', 'done'], { cwd: projectRoot, homeDir })
  ...
  it('supports filtering by executor', async () => {
    ...
    const row = db.prepare('SELECT id FROM task ORDER BY created_at ASC LIMIT 1').get() as {
      id: string
    }
    db.close()
    ...
    const result = await runCli(['task', 'list', '--executor', 'beads'], {
      cwd: projectRoot,
      homeDir,
    })
  ...
  it('supports filtering by source', async () => {
    ...
    const result = await runCli(['task', 'list', '--source', 'scan_suggestion'], {
      cwd: projectRoot,
      homeDir,
    })
  ...
  it('shows external reference markers for imported beads tasks', async () => {
    ...
    const result = await runCli(['task', 'list', '--source', 'beads_sync'], {
      cwd: projectRoot,
      homeDir,
    })
  ...
  it('returns structured json task list output', async () => {
    ...
    const result = await runCli(['task', 'list', '--json'], { cwd: projectRoot, homeDir })
  ...
  it('prints help and accepts fp alias', async () => {
    const result = await runCli(['task', 'list', '--help'], { binName: 'fp' })
  ...
  it('localizes task list help output after language is switched', async () => {
    ...
    const result = await runCli(['task', 'list', '--help'], { homeDir })
  ...
  it('does not leak tasks from another managed project', async () => {
    ...
    const secondOnlyTask = await runCli(['task', 'create', '--title', '只在第二项目出现'], {
