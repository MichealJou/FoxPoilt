import { toJsonSuccessOutput } from '@/cli/json-output.js'
import type { CliResult } from '@/commands/init/init-types.js'
import { collectPlatformRegistry } from '@foxpilot/runtime/read-models/control-plane-registry.js'

import type {
  PlatformsDoctorArgs,
  PlatformsDoctorContext,
  PlatformsDoctorDependencies,
} from '@/commands/platforms/platforms-doctor-types.js'

function getDependencies(
  overrides: Partial<PlatformsDoctorDependencies> = {},
): PlatformsDoctorDependencies {
  return {
    collectPlatformRegistry,
    ...overrides,
  }
}

function buildHelpText(binName: 'foxpilot' | 'fp'): string {
  return [
    '查看平台层健康检查摘要。',
    '',
    `${binName} platforms doctor`,
    `${binName} platforms doctor --json`,
  ].join('\n')
}

export async function runPlatformsDoctorCommand(
  args: PlatformsDoctorArgs,
  context: PlatformsDoctorContext,
): Promise<CliResult> {
  if (args.help) {
    return {
      exitCode: 0,
      stdout: buildHelpText(context.binName),
    }
  }

  const dependencies = getDependencies(context.dependencies)
  const items = await dependencies.collectPlatformRegistry()
  const data = {
    summary: {
      ready: items.filter((item) => item.status === 'ready').length,
      degraded: items.filter((item) => item.status === 'degraded').length,
      unavailable: items.filter((item) => item.status === 'unavailable').length,
    },
    items: items.map((item) => ({
      platformId: item.platformId,
      status: item.status,
      issues: item.healthSummary ? [item.healthSummary] : [],
      suggestedActions: item.availableActions,
    })),
  }

  if (args.json) {
    return {
      exitCode: 0,
      stdout: toJsonSuccessOutput('platforms doctor', data),
    }
  }

  return {
    exitCode: 0,
    stdout: [
      '[FoxPilot] Platforms Doctor',
      `- ready: ${data.summary.ready}`,
      `- degraded: ${data.summary.degraded}`,
      `- unavailable: ${data.summary.unavailable}`,
    ].join('\n'),
  }
}
